...
public class numbersToWords {
    public static void numbersToWord(int number) {
        ...
        int revesedNumber = reverse(number);
        int countReversedNumber = getDigitCount(revesedNumber);
        int diffCheck = 0;

        while(revesedNumber > 0) {
            int lastDigit = revesedNumber % 10;
            switch (lastDigit) {
                case 0:
        ...
        if(countOriginalNumber != countReversedNumber) {
            diffCheck = countOriginalNumber - countReversedNumber;
            for(int i = 0; i < diffCheck; i++) {
                System.out.println("ZERO");
            }
        }
