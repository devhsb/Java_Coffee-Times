...
*getBucketCount(2.75, 3.25, 2.5, 1); → should return 3 since the wall area is 8.9375, a single bucket can cover an
...
Write another overloaded method named getBucketCount with 3 parameters namely width, height, and areaPerBucket (all
of type double).
...
3. In some cases, Bob does not know the width and height of the wall but he knows the area of a wall. He needs you to
...
public class paintJob {
    public static double getPacketCount(double width, double height, double areaPerBucket, int extraBucket) {
        if (width <= 0 || height <= 0 || areaPerBucket <= 0 || extraBucket < 0) {
            return -1;
        }

        double area = width * height;
        double bucketsNeed = area / areaPerBucket;
        bucketsNeed -= extraBucket;
        return Math.ceil(bucketsNeed);
    }

    // method without extra buckets
    public static double getPacketCount(double width, double height, double areaPerBucket) {
    ...
    //method without width and height
    public static double getPacketCount(double area, double areaPerBucket){
        if(area < 0 || areaPerBucket < 0) {
            return -1;
        }else {
            return Math.ceil(area / areaPerBucket);
        }
    }
}
